...
G_Error rendererAllmighty::loadTexture( char* file, fileType type, texture* text )
{
...
//		coLogg( out);
	...
	if (type == TGA)
	{
		...
		if ( err ) {
            ...
            std::cout << out;
//            coLogg( out);
			return G_GLU_ERROR;	//gluBuild failed, return false
		}
		glfwFreeImage( &img );
	...
	if (type==PNG)
	{
		...
		else return G_GLPNG_ERROR;
	}

	return G_NO_SUPPORTED_FORMAT;
}
